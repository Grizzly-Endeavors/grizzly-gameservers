...
The hard constraint: in-game players have **no Discord identity** and no per-caller tenancy. The Discord tenancy boundary (a GameServer is labelled with its owning Discord guild; every read/action confines to the caller's guild) is the guardrail that contains Gary; an in-game entrypoint must not become an unauthenticated path around it, and it opens a new prompt-injection surface (a player controls the text that reaches the model). (This ADR was written when the boundary was the Discord *channel*; [ADR-006](006-guild-tenancy.md) later moved it to the *guild*. The in-game entrypoint resolves whatever that boundary is off the server's own label, so the design is unchanged.)
