...
- `/command` → run one in-game console command over RCON and return the reply. The supervisor **mints an ephemeral RCON password at pod startup** and injects it into the game child's environment (itzg's `RCON_PASSWORD`), then authenticates its own RCON client with the same value — so the password never touches git or a Kubernetes object and rotates each pod start. The per-game template turns RCON on (`SUPERVISOR_RCON_PORT`, plus `SUPERVISOR_RCON_DIALECT` = `minecraft` \| `source` \| `palworld` to select the reply-framing + command vocabulary); the port stays pod-internal. The bot exposes this to admins as Gary's `send_command` tool.
  - **`palworld` dialect** is a real quirk, not cosmetic: Palworld's RCON is a partial Source implementation that replies in one packet and never mirrors the sentinel a correct Source server (Factorio) does, so reading it as base Source hangs until timeout/EOF and reports a false failure on a command that actually ran. The palworld dialect reads a single packet (like Minecraft) and uses Palworld's own `Broadcast` verb.
  - **Palworld config seeding** is the other Palworld-specific wrinkle: its upstream image regenerates the whole `PalWorldSettings.ini` from env on every boot, which wipes any per-instance edit (an unset `SERVER_PASSWORD` resets the join password to blank each restart). The image sets `DISABLE_GENERATE_SETTINGS=true` so the on-PVC ini is authoritative, and the supervisor owns just the RCON keys — re-seeding `RCONEnabled`/`RCONPort`/`AdminPassword` into the ini (`SUPERVISOR_PALWORLD_INI`) before *each* launch — so Gary's file edits persist while RCON still comes up on the minted password. The password still rotates per pod (the freshly minted value overwrites `AdminPassword` on every boot); the only difference from the env-injected games is that for Palworld it lands in the persisted ini rather than only the process environment.
