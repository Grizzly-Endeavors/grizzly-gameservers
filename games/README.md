...
- `minecraft/` — the first entry and the bring-up validation server: a standalone Agones `GameServer` (itzg/minecraft-server + a busybox readiness sidecar that drives the Agones SDK `/ready` + `/health`), a `PersistentVolumeClaim` for world state, and a `NodePort` Service on `7000`. Currently a single live instance; promoting to a `Fleet` + shim-managed per-instance NodePort Services is a follow-up once the shim exists.
