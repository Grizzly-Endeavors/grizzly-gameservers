...
Use `scripts/local-bot.sh {start|stop|restart|status|logs}` (or `just bot-start` / `bot-stop` / `bot-logs`) to run the Discord bot against the cluster in your current kubeconfig context. Secrets come from the repo-root `.env` (loaded by the binary via dotenvy): `DISCORD_BOT_TOKEN` and `GAMESERVERS_ADMIN_USER_IDS` (comma-separated Discord user ids — the **cross-guild operator seed**; admins in every guild). The bot is multi-guild (commands register per-guild on join), so there's no `DISCORD_GUILD_ID`. Per-guild admins (roles and users) are set at runtime with `/config` and stored in Postgres — there's no `GAMESERVERS_ADMIN_ROLE_ID` any more; the guild **owner** is an implicit admin so a fresh guild is usable via `/config` with no env change. To test Gary (the ops agent) locally, also set `OLLAMA_API_KEY` — without it, Gary replies that he isn't configured; optionally override `OLLAMA_BASE_URL` / `OLLAMA_MODEL` (defaults to Ollama Cloud and `glm-5.2`). No-mention **home channels** (`/gary-home`) and per-guild admin `/config` need `DB_PASSWORD` (the foundation-Postgres credential; `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER` default to the R730xd `grizzly_gameservers` role/DB) — omit it and the bot degrades gracefully (home channels off, admin config fail-closed to operators + guild owner) while mentions/DMs/slash commands keep working. The script overrides `GAMESERVERS_CATALOG_DIR` to the in-repo `games/` because the compiled default is the in-container path. It builds, launches the binary detached, writes the pid to `target/local-bot.pid`, and streams output to `target/local-bot.log`. After editing `.env` you must `restart` for changes to take effect.

The bot runs locally for dev (no image needed), but the **per-game supervisor runs in-pod**, so iterating on it means building and pushing the composite game image. Use `scripts/push-game-image.sh [game] [tag]` (or `just game-push minecraft dev`): it builds `games/<game>/Dockerfile` (cargo-chef-cached, so a source-only change rebuilds in seconds), port-forwards the in-cluster registry (`registry.registry.svc.cluster.local:5000`, plain HTTP), and pushes through `localhost:5000` (Docker treats localhost as insecure). The `:dev` catalog tag pins `imagePullPolicy: Always`, so a freshly created (or cold-started) server re-pulls. This is the dev loop; the durable path is the gated CI build (`.github/workflows/deploy.yml`, live — every push to `main` builds, gate-signs, and Flux deploys the bot image). Don't full-send every change through CI — the bot iterates locally in seconds, and only the supervisor needs an image.
