...
- `crates/` — Rust workspace. `bot/` (Discord shim + ops agent + Agones client + config loader, binary `grizzly-gameservers`), `supervisor/` (in-pod process supervisor baked into game images, binary `grizzly-supervisor`), `control-api/` (the wire contract shared in both directions: the bot's control client ↔ the supervisor's control server, plus the reverse in-game trigger path where the bot is the server and the game pod is the client — also the source of truth for the route-path constants like `FS_READ_PATH` and `CONTROL_PORT`, so reach for those instead of a bare literal), `prompt-lib/` (build-time compiler for all model-visible prompt text into typed Rust accessors — see "Prompt Library" below). Strict lints live in `[workspace.lints]` at the root `Cargo.toml`; each crate opts in with `[lints] workspace = true`.
